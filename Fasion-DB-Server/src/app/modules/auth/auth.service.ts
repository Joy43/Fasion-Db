...
const loginUser = async (payload: IAuth) => {
   ...
   try {
      ...
      try {
         ...
         const logMsg = `[${new Date().toISOString()}] SERVICE DEBUG LOGIN: Email: ${payload.email} | PlainPassword: ${payload.password} | DbPassword: ${user.password}\n`;
         await appendFile(path.join(__dirname, "..", "..", "..", "..", "auth_debug.log"), logMsg, "utf8");
      } catch (err) {
         console.error("Failed to write service debug log:", err);
      }
