  ...
  return (
    <SafeAreaView className="flex-1 bg-white">
      <ScrollView className="bg-white">
        ...
        {/* Floating Card */}
        <View className="bg-white -mt-16 rounded-3xl px-6 py-5 shadow-lg mx-4 z-10">
          {/* Thumbnail + Price */}
          <View className="flex-row items-center space-x-4">
            ...
            <View>
              ...
              <View className="flex-row mt-1 space-x-2">
                <Text className="bg-gray-100 px-3 py-1 rounded-lg text-sm">{product.color || 'Pink'}</Text>
        ...
        <View className="mt-6 px-4">
          {/* Key Features */}
          <Text className="text-base font-bold text-gray-800 mb-2">Key Features</Text>
          {(Array.isArray(product.keyFeatures) ? product.keyFeatures : [product.keyFeatures])?.map(
          ...
          {product.specification && (
            <>
              <Text className="mt-4 text-base font-bold text-gray-800 mb-2">Specifications</Text>
              {Array.isArray(product.specification) ? (
                product.specification.map((spec: string, index: number) => (
                  <Text key={index} className="text-gray-600">• {spec}</Text>
                ))
